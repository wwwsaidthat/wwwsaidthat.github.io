...
from pyagc.models import SSGE
from pyagc.transforms import GSSLTransform

from .base_method import BaseMethod


class SSGEMethod(BaseMethod):
    """SSGE 自监督方法。

    SSGE 将 CCA-SSG 的去相关（decorrelation）正则替换为
    均匀性（uniformity）正则：Wasserstein 距离到 N(0, I)。
    """
    ...
    def __init__(
        ...
        super().__init__(method_name="ssge", is_supervised=False)
        encoder = GCN(
            in_channels=in_dim,
        ...
        self.model = SSGE(encoder=encoder, transform1=t1, transform2=t2, lam=lam)
